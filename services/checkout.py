...
def process_signature(token, signature_data, signed_ip):
    ...
    try:
        ...
        from utils.storage import get_checkout_path, ensure_dir
        pdf_dir = ensure_dir(get_checkout_path(record.project))
        ...
        pdf_public_url = f"{base_url}/checkout/document/{rel_pdf_path}"
...
def _trigger_n8n_webhook(inspection_id, rel_pdf_path, base_url, current_hash, data):
    try:
        ...
        pdf_url_signed = f"{base_url}/checkout/document/{rel_pdf_path}?t={token_n8n}"
