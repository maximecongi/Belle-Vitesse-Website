...
def get_vehicle_timeline(vehicle_id: str) -> Optional[Dict[str, Any]]:
    ...
    for inc in incidents:
        ...
        events.append({
            ...
            "status": inc.status,
            "status_label": getattr(inc, "status_label", None) or ("Signalé" if inc.status == "signale" else inc.status.capitalize()),
            "severity": inc.severity,
            "severity_label": getattr(inc, "severity_label", None) or ("Critique" if inc.severity == "critique" else ("Mineur" if inc.severity == "mineur" else "Modéré")),
