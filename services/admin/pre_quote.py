...
def generate_reference():
    """Génère une référence unique séquentielle : PQ-YYYY-NNN."""
    year = datetime.now().year
    
    # On cherche le dernier numéro pour cette année
    prefix = f"PQ-{year}-"
    last_quote = PreQuote.query.filter(PreQuote.reference.like(f"{prefix}%")).order_by(PreQuote.reference.desc()).first()
...
def create_pre_quote(data, user_id=None):
    """Crée une nouvelle pré-quote."""
    reference = generate_reference()
    totals = calculate_totals(data.get('prestations', []), data.get('tva_rate', 20.00))
...
def update_pre_quote(quote_id, data):
    """Met à jour une pré-quote existante."""
    ...
    if 'shoot_date' in data:
        quote.shoot_date = data['shoot_date']
    if 'shoot_location' in data:
        quote.shoot_location = data['shoot_location']
    if 'prestations' in data:
        quote.prestations = data['prestations']
        totals = calculate_totals(quote.prestations, data.get('tva_rate', quote.tva_rate))
...
def get_pre_quote_pdf(quote_id):
    ...
    html = render_template('pdf/pre_quote.html', quote=quote, 
                           now=datetime.now(),
                           settings={
