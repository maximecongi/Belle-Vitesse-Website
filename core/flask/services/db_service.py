...
if DATABASE_URL:
    # Handle mysql+pymysql and mysql schemes
...
# Global tunnel for SSH mode
_ssh_tunnel = None

def init_db_service(app_config, app_cache: Cache):
    """Initialize the database service with app config and cache."""
    global cache
    cache = app_cache
...
def _fetch_all_from_table(table_name, order_by=None):
    ...
    try:
        ...
        cursor.close()
        connection.close()
        if tunnel and USE_SSH_TUNNEL is False: # Only stop if it's not the global one (wait, logic check)
            pass # Keep global tunnel open for performance if needed, but per-request link is safer here
...
def get_vehicle_by_slug(slug):
    return get_cached(f"vehicle_{slug}", lambda: _fetch_by_field("vehicles", "slug", slug))
...
def get_grips_categories_by_slug(slug):
    return get_cached(f"grips_categories_{slug}", lambda: _fetch_by_field("grips_categories", "slug", slug))
...
def warm_cache(app):
    with app.app_context():
        try:
            ...
            app.logger.info("🔥 DB Cache warmed successfully")
        except Exception as e:
            app.logger.error(f"❌ Error warming DB cache: {e}")
