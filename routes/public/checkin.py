...
def init_checkin_routes(app):
    ...
    def checkin_generate():
        ...
        result = generate_inspection_token(payload["record_id"], "checkin")
        if not result:
    ...
    def checkin_sign_page(token):
        ...
        return render_template("public/inspection_sign.html", data=data, token=token, type="checkin")

    @app.route("/checkin/sign/<token>/abandon", methods=["POST"])
    @csrf.exempt
    def checkin_abandon(token):
        abandon_inspection_signature(token, "checkin")
