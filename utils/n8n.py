...
def trigger_n8n_webhook(url: str, **kwargs) -> bool:
    """
    Trigger an n8n webhook with the given URL and arguments as payload.
    ...
    doc_id = kwargs.get("inspection_id") or kwargs.get("id", "")
    log_suffix = f" for {doc_id}" if doc_id else ""

    logger.info(f"🚀 Triggering n8n webhook{log_suffix}...")

    try:
        response = http_requests.post(url, json=kwargs, timeout=10)
